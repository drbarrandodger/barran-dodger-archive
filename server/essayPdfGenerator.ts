...
export function generateEssayPDF(essay: CosmicEssayEntry): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    if (fs.existsSync(coverPath)) {
      ...
      // Dark overlay
      doc.rect(0, 0, pageW, pageH).fillOpacity(0.55).fill("#000000").fillOpacity(1);
    } else {
      doc.rect(0, 0, pageW, pageH).fill("#050500");
    }
    ...
    doc.rect(margin - 8, margin - 8, bodyW + 16, 20).fill("#1c0a00");
    doc.font("Helvetica-Bold")
      .fontSize(8)
      .fillColor("#b45309")
      .fillOpacity(1)
