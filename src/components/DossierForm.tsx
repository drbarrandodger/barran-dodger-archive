...
// Mock documents for the dropdown if DB is empty
const MOCK_DOCS = [
  { id: '1', title: 'Suppressed Forensic Report #788', agency: 'Department of Justice' },
  { id: '2', title: 'Email Correspondence: Operation Barran', agency: 'Intelligence Services' },
  { id: '3', title: 'Financial Audit of Suppressed Funds', agency: 'Treasury' },
];
...
const DossierForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const docTitle = MOCK_DOCS.find(d => d.id === selectedDocId)?.title || customAgency;
    ...
    return (
      <div className="bg-gray-900 border border-justice-gold/30 p-8 rounded-xl text-center max-w-2xl mx-auto my-12">
        ...
        <p className="text-gray-400 mb-6">
          Your AI Forensic Dossier request has been received. Our forensic engine is now synthesizing the evidence.
          You will receive the report at <strong>{email}</strong> within 24 hours.
        </p>
        <button 
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          >
            ...
            {MOCK_DOCS.map(doc => (
              <option key={doc.id} value={doc.id}>{doc.title} ({doc.agency})</option>
            ))}
            <option value="custom">Other / Specific Agency...</option>
          </select>
        </div>
